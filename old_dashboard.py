...
# Função para carregar os dados com cache leve
@st.cache_data(ttl=INTERVALO_ATUALIZACAO)
def carregar_dados():
    return pd.read_csv(METRICAS_FILEPATH)
...
def display_metrics():
    ...
    st.metric("Tempo Médio de Sessão (s)", f"{avg_session_time:.2f} s")

    # Tempo de sessão total
    total_session_time = np.sum(data['tempo_da_sessao']/60)
    st.metric("Tempo Total da Sessão (min)", f"{total_session_time:.2f} min")
